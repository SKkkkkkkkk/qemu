...
static int nds_ace_get_reg(CPUState *cs, int reg_n)
{
    RISCVCPUClass *mcc = RISCV_CPU_GET_CLASS(cs);
    RISCVCPU *cpu = RISCV_CPU(cs);
    CPURISCVState *env = &cpu->env;
...
static int nds_ace_set_reg(CPUState *cs, unsigned char *val, int reg_n)
{
    RISCVCPUClass *mcc = RISCV_CPU_GET_CLASS(cs);
    RISCVCPU *cpu = RISCV_CPU(cs);
    CPURISCVState *env = &cpu->env;
...
{
    unsigned total_bytes = nds_ace_get_reg(cs, n);
    if (total_bytes != -1) {
...
{
    unsigned total_bytes = nds_ace_set_reg(cs, mem_buf, n);
    if (total_bytes != -1) {
        return total_bytes;
    }
    return 0;
